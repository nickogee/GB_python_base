...
'''
 п. 6. * Реализовать структуру данных «Товары». Она должна представлять собой список кортежей. 
...
# Зададим постянные значения в качестве констант
KEYS = ['название', 'цена', 'количество', 'ед.изм']
YES_LS = ['да', 'д', 'yes', 'y', 'ok', 'ок']
NO_LS = ['нет', 'н', 'no', 'n']

# В цикле будем запрашивать у пользователя данные для базы товаров
n = 1           # счетчик тваров
base = []       # структура для хранения ответов пользователя (база товаров)

done = False
while not done:
    answr = input('Добавить товар в базу? (Да/Нет)\n').lower()
    if answr in YES_LS:
        spec = {}
        for key in KEYS:
            spec[key] = input(f'Укажите характеристику "{key}"\n')
...
# Соберем и выведем аналитику
if base:
    struct = {}
    for key in KEYS:
        struct[key] = [i[1][key] for i in base]
        print(key, ': ', struct[key], sep='')  # Сразу в цикле построчно выводить красивее
    # print(struct)
else:
