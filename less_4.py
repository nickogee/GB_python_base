...
def print_step(my_list, lim, reverce = False):
    '''
    печатает элементы списка в цикле, пока количество не достигнет lim
    :param my_list: -> (list)
    :param lim: -> (int)
    :return:
    '''
...
# здесь прошла птичка
print_step(ls, step_lim)
print_step(ls, step_lim, True)
...
def fact(n):
    '''
    вычисляет факториалы чисел до значения n и возвращает генератор, итерируемый по вычисленным значениям факториалов
    :param n: -> (int)
    :return: generator
    '''
    ls = [factorial(i) for i in range(1, n+1)]
    for el in ls:
        yield el


# применяем созданный генератор
for i in fact(6):
    print(i)
